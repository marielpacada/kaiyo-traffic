...
```{r covid-ratios}
sales_total <- length(third_of_year$status[third_of_year$status == "Sold"])
sales_before <- length(pre_covid$status[pre_covid$status == "Sold"])
sales_during <- length(covid$status[covid$status == "Sold"])

rec_total <- length(third_of_year$status[third_of_year$status == "Receiving"])
rec_before <- length(pre_covid$status[pre_covid$status == "Receiving"])
rec_during <- length(covid$status[covid$status == "Receiving"])

# this is two-thirds :) 
sales_before / sales_total
sales_during / sales_total
...
rec_before / rec_total
rec_during / rec_total
```
