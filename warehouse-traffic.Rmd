...
ggplot(first_tri, aes(x = category, y = count)) + geom_bar(fill = "#F2AE0F", stat = "identity") + 
  labs(title = "First Trimester Receiving")
```

```{r likelihood-sold}
leave_prob <- function(activity) {
  status_data <- warehouse %>% filter(status == activity) %>% group_by(category) %>% summarize(count = n())
  rec_data <- warehouse %>% filter(status == "Receiving") %>% group_by(category) %>% summarize(count = n())
  df <- data.frame(category = levels(categories$category), ratio = status_data$count / rec_data$count)
  
  return(ggplot(df, aes(x = category, y = ratio)) + geom_bar(fill = "#F2AE0F", stat = "identity") + 
           labs(title = paste("Probability of Being", activity)))
}
...
general_activity <- warehouse %>% filter(year != "2020") %>% group_by(year, status) %>% summarize(count = n())
general_yearly <- general_activity %>% group_by(year) %>% summarize(count = sum(count))
